...
export default function Login() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      email: Yup.string()
        .required("Bắt buộc.")
        .test("is-email-or-username", "Email hoặc tên đăng nhập không hợp lệ.", (value) => {
      ...
      password: Yup.string().required("Bắt buộc.").min(3, "Phải chứa ít nhất 3 ký tự."),
    }),
  });
    ...
    try {
      ...
      if (data.status === 200) {
        ...
        const userExists = users.find(
          (user) => user.username === formik.values.email || user.email === formik.values.email
        );
  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: "#f8f8f8", padding: "100px 0" }}>
        <Container className="login__container" maxWidth="xl" style={{ width: "1280px" }}>
          <Grid container spacing={3} className="login__content">
            <Grid item md={6} className="login__image">
              <figure>
                <img src="/assets/images/login-image.jpg" alt="A guy typing a laptop" />
              </figure>
            </Grid>
            <Grid item md={6} className="login__form">
              <Typography className="loginForm__title" fontWeight={"bold"} variant="h1" mb={2}>
                Đăng nhập
              </Typography>
              ...
              <form className="loginForm" id="loginForm" onSubmit={formik.handleSubmit}>
                <div className="form-textField">
                  <FormControl fullWidth>
                    <InputLabel htmlFor="outlined-adornment-Tên_đăng_nhập">Tên đăng nhập hoặc Email</InputLabel>
                    <OutlinedInput
